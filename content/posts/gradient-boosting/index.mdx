---
title: "ML Part 3: Gradient Boosting desde 0 en Python"
author: Cristobal Aguirre
date: 2019-06-26
hero: "./golf-MSE.PNG"
excerpt: "El final en esta serie de machine learning que introduce Gradient Boosting&mdash;el algoritmo más exitoso de los últimos años. En este post, voy a describir un poco la teoría para luego construir una implementación en Python desde 0, que en solo 60 líneas logra resultados comparables con implementaciones reales como XGBoost y Scikit-learn."
---
...
estimador corrigiendo los errores (figurativa y a veces literalmente) del estimador anterior. El _Boosting_ en el
nombre del algoritmo viene de este proceso aditivo para juntar estimadores débiles, mientras que el _Gradient_ viene
del hecho que se usa la técnica conocida en inglés como _Gradient Descent_ para la optimización. Como se ve en esta
imagen que saque del tercer post en la serie de 3 recomendada más arriba, el proceso consiste en estimar
secuencialmente estimadores que van acercándose de a poco a la meta, usando el negativo del gradiente de la función de
pérdida (el gradiente es el vector de derivadas, que en este caso se puede interpretar mucho más fácilmente como los _pseudo-residuos_) evaluada en el paso anterior para estimar el siguiente estimador (más adelante viene la explicación detallada).

![Ejemplo de gradient descent con golfista](golf-MSE.PNG)

Es mucho más fácil interpretar esto tomando el error cuadrático medio como función de pérdida, ya que su gradiente es equivalente a los residuos comunes $y - f(x)$ (en lo que sigue, voy a referirme solo al caso de regresión y a estos simplemente como los residuos para hacer más fácil la interpretación.) Entonces, en cada paso $m$ se calculan los residuos entre la variable objetivo y la estimación del paso anterior $f_{m-1}$, y se estima un nuevo estimador $f_m$ que busque “explicar” estos residuos.

Al final del proceso, el estimador final $F_m(x)$ consistirá en un estimador base $f_0$, que puede ser una serie de 0’s, el promedio, la mediana, etc., y una sumatoria de $m$ estimadores débiles $f_m(x)$, ajustados por un parámetro de regulación $\eta$ que afecta la contribución de cada estimador al estimador final (conocido en inglés como _shrinkage_ o _learning rate_).
...
En el caso del error cuadrático medio, el algoritmo consiste en:

1. Estimar un estimador base $F_{0}$
...
def mse_gradient(y, f):
    """
    Error cuadrático, cuyo gradiente
...
```python
# Voy a ignorar los imports en los segmentos de codigo mas abajo
...
# BaseEstimator provee algunos metodos de utilidad no usados aca
...
Con estos dos métodos finales terminamos la implementación, que con toda la funcionalidad básica y desempeño comparable a las de _full scale_, pero sin los controles y optimizaciones, alcanza apenas 60 líneas.
