---
title: "Descargando información histórica de Twitter con Python"
date: 2019-06-04
author: Cristobal Aguirre
hero: "../../../src/assets/random-code.PNG"
---
...
Para usar el programa, lo primero que tienes que hacer es clonar el repositorio a tu computador local. Luego, hay que crear un archivo llamado `secrets.py` donde pondrás tus credenciales de Twitter-developer. Este archivo tiene que tener la siguiente estructura:
...
# Reemplaza con tus claves individuales
api_key = ApiKey(
    'consumer-key',
    'consumer-secret',
    'access-token',
...
Si te manejas un poco con python y no te gusta esta estructura, puedes reemplazarlo con variables de entorno (_environment variables_ en inglés) definidas en un archivo `.env` e importadas con una librería como [`python-decouple`](https://github.com/henriquebastos/python-decouple) (si usas Django ya sabes este proceso). Eso si, no te olvides de agregarlo al `.gitignore` para que no te lo incluya en git—no quieres tus claves y tokens secretos disponibles en github!
